...
public class StudentBizImpl implements StudentBiz{
	...
	admins[4] = x5;
	 ...
	 public void viewStudent() {
			System.out.println(" "+"id"+" "+"年龄"+"   "+"姓名"+"   "+"性别"+"  "+"年级        "+"成绩"+"\r\n");
			...
			for(int i=0;i<person.length-1;i++){  
				...
				for(int j = i+1;j < person.length;j++) {
					if(person[j] != null) {
					if(person[i].scores < person[j].scores) {
	...
	public void changeStudent(int id, int age, String name, String sex, String _class,int scores)  {
	...
	public void addStudent(int id, int age, String name, String sex, String _class,int scores) {
		...
		Student x = new Student(id, age, name, sex,_class,scores);
		for( int i=0;i<person.length;i++) {
			if(person[i] != null) {
				continue;
			}else{ 
